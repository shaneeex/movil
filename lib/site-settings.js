...
function normalizeHeroVideoOpacity(raw) {
  return normalizeHeroOpacityValue(raw, HERO_VIDEO_OPACITY_MIN, HERO_VIDEO_OPACITY_MAX, DEFAULT_HERO_VIDEO_OPACITY);
}

function normalizeHeroVideoBrightness(raw) {
  return normalizeHeroOpacityValue(
    raw,
    HERO_VIDEO_BRIGHTNESS_MIN,
    HERO_VIDEO_BRIGHTNESS_MAX,
    DEFAULT_HERO_VIDEO_BRIGHTNESS,
  );
}

function normalizeHeroVideoSaturation(raw) {
  return normalizeHeroOpacityValue(
    raw,
    HERO_VIDEO_SATURATION_MIN,
    HERO_VIDEO_SATURATION_MAX,
    DEFAULT_HERO_VIDEO_SATURATION,
  );
}

function normalizeHeroVideoContrast(raw) {
  return normalizeHeroOpacityValue(
    raw,
    HERO_VIDEO_CONTRAST_MIN,
    HERO_VIDEO_CONTRAST_MAX,
    DEFAULT_HERO_VIDEO_CONTRAST,
  );
}

function normalizeHeroVideoBlur(raw) {
  return normalizeHeroOpacityValue(raw, HERO_VIDEO_BLUR_MIN, HERO_VIDEO_BLUR_MAX, DEFAULT_HERO_VIDEO_BLUR);
}

function normalizeHeroVignetteOpacity(raw) {
  return normalizeHeroOpacityValue(
    raw,
    HERO_VIGNETTE_OPACITY_MIN,
    HERO_VIGNETTE_OPACITY_MAX,
    DEFAULT_HERO_VIGNETTE_OPACITY,
  );
}
...
function normalizeHeroVideo(entry) {
  ...
  const videoOpacity = normalizeHeroVideoOpacity(entry.videoOpacity);
  const videoBrightness = normalizeHeroVideoBrightness(entry.videoBrightness);
  const videoSaturation = normalizeHeroVideoSaturation(entry.videoSaturation);
  const videoContrast = normalizeHeroVideoContrast(entry.videoContrast);
  const videoBlur = normalizeHeroVideoBlur(entry.videoBlur);
  const vignetteOpacity = normalizeHeroVignetteOpacity(entry.vignetteOpacity);
  return {
    ...sanitized,
